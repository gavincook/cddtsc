...
public class FileUploadAction implements InitializingBean,ApplicationContextAware{
        ...
        for(MultipartFile file:files){
            ...
            try{
                ...
                if(isAvatar){//如果是头像需要保存一个200*200的尺寸，并且将200*200设为默认
                    ...
                    BufferedImage image200 = Images.scaleImage(originImage,200,200,true);
                    ImageIO.write(image200,"jpg",out);
                    BufferedImage image100 = Images.scaleImage(originImage,100,100,true);
    ...
    /**
     * Base64图片上传，返回格式见#{@link #uploadFile(org.springframework.web.multipart.MultipartFile[])}
     * @param content
     * @return
     */
